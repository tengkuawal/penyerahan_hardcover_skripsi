...
@section('title', 'Tambah Penyerahan')
@section('page_heading', 'Catat Penyerahan Hardcover Baru')
    ...
    <div class="col-md-8">
        <div class="card card-custom">
            <div class="card-header bg-white py-3">
                <h6 class="fw-bold mb-0 text-slate-800"><i class="bi bi-journal-plus me-2 text-primary"></i> Form Penyerahan Hardcover</h6>
            </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="tipe" class="form-select" required>
                                ...
                                <option value="ta" {{ old('tipe') == 'ta' ? 'selected' : '' }}>TA (Tugas Akhir - Cover Blue)</option>
                            </select>
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold">Judul Hardcover <span class="text-danger">*</span></label>
                            <textarea name="judul" class="form-control" rows="2" placeholder="Masukkan judul lengkap Skripsi / KKP / TA..." required>{{ old('judul') }}</textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Tanggal Penyerahan</label>
                            <input type="date" name="tanggal_penyerahan" class="form-control" value="{{ old('tanggal_penyerahan', date('Y-m-d')) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Status Penyerahan <span class="text-danger">*</span></label>
                            <select name="status" class="form-select" required>
                                <option value="Sudah Menyerahkan" {{ old('status') == 'Sudah Menyerahkan' ? 'selected' : '' }}>Sudah Menyerahkan</option>
                                <option value="Belum Menyerahkan" {{ old('status') == 'Belum Menyerahkan' ? 'selected' : '' }}>Belum Menyerahkan</option>
                            </select>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label fw-semibold">Petugas Penerima</label>
                            <input type="text" name="petugas_penerima" class="form-control" value="{{ old('petugas_penerima', Auth::user()->name ?? 'Petugas Staff') }}" placeholder="Nama Petugas Penerima Hardcover">
                        </div>
                    </div>

                    <div class="d-flex justify-content-end gap-2 mt-4">
                        <a href="{{ route('submissions.index') }}" class="btn btn-light border px-4">Batal</a>
                        <button type="submit" class="btn btn-primary-custom px-4">Simpan Penyerahan</button>
                    </div>
                </form>
            </div>
        </div>
